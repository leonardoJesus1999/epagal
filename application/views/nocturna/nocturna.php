...
<body>
            ...
            <div class="row column_title">
                <div class="col-md-12">
                    <div class="page_title">
                        <h2>Mapa de Ecotachos Nocturnos</h2>
                    </div>
                </div>
            </div>

            <div id="mapaNocturna"></div>
        </div>
    </div>
    ...
    <script>
        function initMap() {
            ...
            var mapaNocturna = new google.maps.Map(document.getElementById('mapaNocturna'), {
                center: coordenadaCentral,
                zoom: 17,
                mapTypeId: google.maps.MapTypeId.ROADMAP
            });

            <?php if (isset($nocturnos) && count($nocturnos) > 0): ?>
                <?php foreach ($nocturnos as $temporal): ?>
                    var coordenadaTemporal = new google.maps.LatLng(<?php echo $temporal->latitud; ?>, <?php echo $temporal->longitud; ?>);
                    var icono = {
                        url: "<?php echo base_url('assets/images/ecotacho3.png'); ?>",
