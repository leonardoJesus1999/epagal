...
(function ($) {
			...
			try {
				...
				$.each(literal, function (key, item) {
					if ($.type(item) === "function") {
						...
						$.each(
							$.validate.validator(property, item),
							function (index, item) {
								array.push(item);
				...
				$.each(array, function (index, item) {
					if (item.type === "object") {
						item.property.selector
							.parent()
							.find("." + options.bootstrap.feedback)
							.remove();
					}
				});
			}
				...
				if (!item.result && item.type === "object") {
					...
					if (options.mode === "alert") {
						...
						if (item.literal.feedback !== undefined) {
							...
							if (
								item.property.type !== "radio" &&
								item.property.type !== "checkbox"
							) {
								$selector.parent().append(
									$("<div />", {
										addClass: options.bootstrap.feedback,
										text: item.literal.message,
									})
								);
							}
						}
						$selector.addClass(options.bootstrap.error);
						$selector.trigger(item.property.event, [
							item.checker,
							item.literal,
							item.property,
						]);
		...
		popup: function (url, name, width, height) {
			...
			window.open(
				...
				"width=" +
					...
					", top=" +
					y +
					", scrollbars=yes, toolbar=no, menubar=no, location=no, resizable=no"
			);
		},
	};
	...
	$.sns = {
		...
		naver: function (msg, url) {
			...
			msg = encodeURI(msg);
			window.open(
				"https://share.naver.com/web/shareView.nhn?url=" + url + "&title=" + msg
			);
		},
	};
	...
	function validatorResult(checker, value1, value2) {
		var result = false;
		if (
			$.library.in(["required", "digit", "email", "date", "mobile"], checker)
		) {
